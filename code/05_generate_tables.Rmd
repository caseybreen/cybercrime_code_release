...
table_s6 <- adjustments_individual %>% 
  ...
  select(Cybercrime = scam, Prevalence = prevalence, `Degree Ratio` = degree_ratio, 
...
table_s7 <- adjustments_pooled %>%
  mutate(`Adjustment Factor` = (1/degree_ratio) * (1/true_positive_rate)) %>% 
  select(Category = factor, `Degree Ratio` = degree_ratio, `Visibility Factor` = true_positive_rate, `Adjustment Factor`) %>% 
  mutate(across(where(is.numeric), round, 5)) %>% 
  gt()
