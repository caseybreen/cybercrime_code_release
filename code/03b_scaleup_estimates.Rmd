...
Network Scale-up Estimates — No Response Verification 
...
basic_nsum_results <- survey_subsample %>% 
  summarize(banking = sum(scaleup_banking*weight)/sum(degree*weight),
            banking_lostmoney = sum(scaleup_banking_desc_lost_money_count*weight, na.rm = T)/sum(degree*weight),
            nondelivery = sum(scaleup_nondelivery*weight)/sum(degree*weight),
            nonpayment = sum(scaleup_nonpayment*weight)/sum(degree*weight),
            overpayment = sum(scaleup_overpayment*weight)/sum(degree*weight),
            advanced_fee = sum(scaleup_advanced_fee*weight)/sum(degree*weight),
            extortion = sum(scaleup_extortion*weight)/sum(degree*weight)) %>% 
  mutate(id = "id") %>% 
  pivot_longer(-id) %>% 
...
true_positive_oversample <- survey %>%  
  filter(!id %in% c("K9T5J5", "Y3A8A2", "O7M7F9", "K2Q0W1", "F6Y1X2")) %>% 
...
write_csv(scaleup_adjustment_factors_pooled, "../data/results/nsum_pooled_adjustments_nofilter.csv")
write_csv(scaleup_adjustment_factors_individual, "../data/results/nsum_individual_adjustments_nofilter.csv")
write_csv(gnsum_bootstrap_estimates, "../data/results/gnsum_estimates_nofilter.csv")
```
